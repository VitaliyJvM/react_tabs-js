...
function getCorrectActiveTabId(tabs, activeTabId) {
    ...
    <div className="tabs is-boxed">
      <ul>
        {tabs.map(tab => (
          <li
            className={cn({
              'is-active': tab.id === getCorrectActiveTabId(tabs, activeTabId),
            })}
            data-cy="Tab"
            key={tab.id}
          >
            <a
              ...
              data-cy="TabLink"
              onClick={() =>
                handleTabSelected(activeTabId, tab.id, onTabSelected)
              }
            >
              {tab.title}
            </a>
